...
# ============================================================================
# DECISION RULE BLOCK – maps raw response to final status (FIX)
# ============================================================================
def categorize_response(response_text):
    ...
    # 6. Everything else = site/proxy error → retry
    return 'ERROR'
        ...
        for _ in range(max_retries):
            ...
            while available_proxies:
                ...
                try:
                    ...
                    # Use the fixed categorizer to decide if it's a gateway response
                    final_status = categorize_response(response_text)

                    if final_status != 'ERROR':
                        # Map APPROVED_OTP to 'APPROVED' (old code compatibility)
                        mapped_status = 'APPROVED' if final_status in ('APPROVED', 'APPROVED_OTP') else final_status
            ...
            # All proxies for this site failed; site already banned, loop will try another site
            continue
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                if card_stopped:
                    ...
                    break
                status = res['status']
                # Existing classification based on response content (cooked vs approved)
                if status == 'APPROVED':
...
def setup_complete_handler(bot, get_filtered_sites_func, proxies_data,
    ...
    def handle_vbv_check(message):
        ...
        vbv_result = vbv_checker(cc)  # now rich dict
        bin_info = get_bin_info(cc.split('|')[0])
